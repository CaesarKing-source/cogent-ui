...
node* getnode()                       //get node frm avail list and node* is the returntype of function like int and void 
{
 node *n1;                   
...
node* create()                        // to create linked list
{
 ...
 ans=getche();              // it takes char as input which will be visible on screen
 while(ans=='y')
 {
 printf("\n Enter the element\n");
 scanf("%d",&element);
 ...
 getch();       // it takes char as input which will be not visible on screen
 return start;
 }
